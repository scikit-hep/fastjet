...
template <typename T>
T swigtocpp(py::object obj) { // unwraps python object to get the cpp pointer from the
                              // swig bindings
...
output_wrapper
interfacemulti(py::array_t<double, py::array::c_style | py::array::forcecast> pxi,
...
PYBIND11_MODULE(_ext, m) {
    ...
    py::class_<output_wrapper>(m, "output_wrapper")
        ...
        .def(
            ...
            [](const output_wrapper ow, double min_pt = 0) {
                ...
                auto E       = py::array(py::buffer_info(nullptr,
                                                   sizeof(double),
                                                   py::format_descriptor<double>::value,
                                                   1,
                                                   {jk},
                ...
                auto off    = py::array(py::buffer_info(nullptr,
                                                     sizeof(int),
    ...
    py::class_<ClusterSequence>(m, "ClusterSequence")
        .def(py::init<const std::vector<PseudoJet>&,
                      const JetDefinition&,
                      const bool&>(),
             "pseudojets"_a,
             "jet_definition"_a,
             "write_out_combinations"_a = false,
             "Create a ClusterSequence, starting from the supplied set of PseudoJets "
             "and clustering them with jet definition specified by jet_definition "
             "(which also specifies the clustering strategy)");
}
