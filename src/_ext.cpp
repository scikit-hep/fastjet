  ...
  py::class_<output_wrapper>(m, "output_wrapper")
    ...
    .def("to_numpy_jet_scale_for_algorithm",
      ...
      [](const output_wrapper ow, const int n_jets = 1, double beta = 0, double symmetry_cut = 0.1,
        ...
        std::vector<uint8_t> uint8_mask(bool_mask.size());
        std::transform(bool_mask.begin(), bool_mask.end(), uint8_mask.begin(),
                      [](bool value) { return static_cast<uint8_t>(value); });

        auto bools_array = py::array_t<uint8_t>(bool_mask.size(), uint8_mask.data());
        ...
        Returns:
          Returns an boolean mask of PF candidates that have been groomed by softdrop.
      )pbdoc")
